...
## Task Master Safety Rules - PREVENT OVER-ENGINEERING

### 🚨 CRITICAL: Task Master Complexity Limits
**WARNING**: Task Master can turn a 200-line task into 25,000 lines if misused!

- **NEVER** use `task-master analyze-complexity` - it creates exponential task growth
- **NEVER** use `task-master expand --all` - it over-engineers simple tasks
- **MAX 7 tasks total** for any project
- **MAX 3 subtasks per task** (no sub-subtasks allowed)
- **If >20 total items**, STOP and simplify immediately
- **If task suggests "framework", "system", or "comprehensive"**, reject it

### Safe Task Master Usage
```bash
# SAFE commands:
task-master list                    # Check task count
task-master add-task --prompt="Simple task description"  # Add one task
task-master set-status --id=1 --status=done  # Mark complete

# DANGEROUS commands (avoid):
task-master analyze-complexity     # ❌ Creates too many tasks
task-master expand --all          # ❌ Over-engineers everything
task-master expand --id=X --research  # ❌ Adds unnecessary complexity
```

### Task Complexity Red Flags
Before accepting any Task Master suggestion, if it contains these words, REJECT IT:
- "comprehensive", "advanced", "framework", "system", "enterprise"
- "factory", "strategy", "observer" (design patterns)
- "architecture", "abstraction", "orchestration"
- Any task with >3 subtasks
- Any subtask that could be a task itself

### The Golden Rule
> **"If Task Master suggests more than 10 total work items, it's wrong."**

Remember: Task Master is a todo list, not an architect. Every task becomes code, and every line of code costs tokens in future LLM conversations.